function[lny,sigma,tau,phi] = MontalvaBastias2017(To,M,R,h,mechanism,region,Vs30)
...
if T_lo==T_hi
    [lny,sigma,tau,phi] = gmpe(index,M,R,h,mechanism,region,Vs30);
else
    [lny_lo,sigma_lo,tau_lo] = gmpe(index,  M,R,h,mechanism,region,Vs30);
    [lny_hi,sigma_hi,tau_hi] = gmpe(index+1,M,R,h,mechanism,region,Vs30);
    x          = log([T_lo;T_hi]);
    Y_sa       = [lny_lo,lny_hi]';
...
function[lnSa,sigma,tau,sig]=gmpe(index,M,R,h,mechanism,region,Vs30)
